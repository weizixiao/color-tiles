...
An asset-free, framework-free, build-free implementation inspired by the gameplay of Google Color Tiles. This open-source version imitates the core puzzle mechanics, keeps only 10 levels, and focuses on making the game rules, level generation, and deterministic random algorithm easy to read and modify.

![Algorithm overview](docs/algorithm-overview.png)

The game itself uses no image assets. The board and pieces are drawn with Canvas color blocks. The image above is only README documentation artwork generated with `gpt-image-2`.
